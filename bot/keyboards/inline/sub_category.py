...
def get_sub_cats(cat_id: int, data) -> InlineKeyboardMarkup:
    ...
    Args:
        ...
        data: A list of subcategory objects, each with a name and ID.
    ...
    for sub in data:
        builder.button(
            text=sub.name,
            callback_data=SubCategoryCallback(id=sub.id, action="view").pack(),
        )
    builder.button(text=" ", callback_data="#") if len(data) % 2 != 0 else None
    builder.button(
        text=BACK_BTN, callback_data=CategoryCallback(id=cat_id, action="back").pack()
    )
    builder.adjust(2)
